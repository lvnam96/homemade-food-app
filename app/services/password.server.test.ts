...
import { comparePassword, getSaltedPassword, hashPassword } from './password.server';
...
describe('Password service:', () => {
  ...
  describe('bcrypt:', () => {
    it('should be able to hash & compare passwords using bcrypt', async () => {
      ...
      const hash2 = await hashPassword(plainPasswd);
      expect(hash1 !== hash2).toBe(true);
      expect(await comparePassword(plainPasswd, hash1)).toBe(true);
      expect(await comparePassword(plainPasswd, hash2)).toBe(true);
    });
  });
